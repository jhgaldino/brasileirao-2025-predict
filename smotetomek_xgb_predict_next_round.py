...
def train_model(X, y):
    ...
    model = XGBClassifier(
        ...
        colsample_bytree=0.9442959401809156,
        learning_rate=0.021272203659117653,
        max_depth=13,
        n_estimators=688,
        subsample=0.7906426798111427,
...
def predict_next_round(model, scaler, next_round_file, live_stats, historical_df, feature_columns):
    ...
    try:
        ...
        for i, match in enumerate(next_round['partidas']):
            ...
            prediction_index = np.argmax(probabilities[i])
            prediction = CLASS_MAP_INV[prediction_index]
...
def main():
    ...
    if df is not None:
        X, y, feature_columns, live_stats, _ = prepare_features_iterative(df)
        y = y.map(CLASS_MAP)
